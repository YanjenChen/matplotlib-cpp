...
PyObject* get_2darray(const Numeric* vp, const size_t& n0, const size_t& n1) {
    ...
    if (type == NPY_NOTYPE) {
        ...
        double* dp = static_cast<double*>(::malloc(memsize));
        for (size_t i = 0; i < arrsize; ++i) dp[i] = vp[i];
...
template <typename NumericX, typename NumericY, typename NumericU, typename NumericW>
bool quiver(const NumericX* x, const NumericY* y, const NumericU* u, const NumericW* w,
...
template <typename NumericX, typename NumericY, typename NumericZ, typename NumericU,
          typename NumericW, typename NumericV>
bool quiver(const NumericX* x, const NumericY* y, const NumericZ* z, const NumericU* u,
            const NumericW* w, const NumericV* v, const size_t& n0, const size_t& n1,
            const size_t& n2, const std::map<std::string, std::string>& keywords = {}) {
    // set up 3d axes stuff
    static PyObject *mpl_toolkitsmod = nullptr, *axis3dmod = nullptr;
    if (!mpl_toolkitsmod) {
        detail::_interpreter::get();
